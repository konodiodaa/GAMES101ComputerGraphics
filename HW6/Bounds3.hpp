                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    for(int i=0;i<3;i++)
    {
        float min_tmp = (pMin[i] - ray.origin[i]) * invDir[i];
        float max_tmp = (pMax[i] - ray.origin[i]) * invDir[i];
        if(!dirIsNeg[i])
        {
            t_enter = std::max(t_enter, max_tmp);
            t_exit = std::min(t_exit,min_tmp );
        }
        else 
        {   
            t_enter = std::max(t_enter,min_tmp );
            t_exit = std::min(t_exit,max_tmp);
        }
    }
